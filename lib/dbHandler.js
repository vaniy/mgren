    ...
    mongoClient.connect(mogoUrl, function (err, db) {
        db.collection('product').find().toArray(function (err, doc) {
            if (err) {
                ...
                db.collection('category').find().sort({ priority: 1 }).toArray(function (err, doc) {
                    let ss = doc;
                    let out = [];
                    ss.forEach(function (child, index) {
                        ...
                        store.forEach(function (cld, idx) {
                            if (!has) {
                                if (child.storeId == cld.storeId && !req.query.isMenu) {
                                    out.push({
                    ...
                    if (!req.query.isMenu) {
                        ...
                        res.send(viewHandler.buildMenuListView(out));
                    }
                    db.close();
                })
            }
        ...
        db.collection('cart').find({ $or: cids }).toArray(function (err, doc) {
            if (doc && doc.length > 0) {
                ...
                db.collection('order').insertOne({
                    ...
                    oid: oid,
                    status: 0,//待确认
                    deliverStatusInfo: { delivers: [{ status: '提交订单', time: moment().format('YYYY-MM-DD HH:MM') }], company: {} },
                    trackingInfo: {},
                    billInfo: {},
                }, function () {
                ...
                res.send({ status: 'failed', msg: '请选择产品' })
                db.close();
            }
        })
    })
...
var getUserOrder = function (req, res, route) {
    ...
    mongoClient.connect(mogoUrl, function (err, db) {
        db.collection('order').find(query).sort({ inDate: -1 }).toArray(function (err, doc) {
            if (err) {
...
var createVideo = function (req, res, product) {
    mongoClient.connect(mogoUrl, function (err, db) {
        var cursor = db.collection('video').find({ "vid": product.vid }).toArray(function (err, doc) {
            if (err) {
                db.close();
            } else if (!doc || doc.length == 0) {
                // var a = product;
...
var createIntegrator = function (req, res, integrator) {
    integrator.priority = parseInt(integrator.priority) || 1;
    mongoClient.connect(mogoUrl, function (err, db) {
...
var getAllCategory = function (req, res, route) {
    mongoClient.connect(mogoUrl, function (err, db) {
        var cursor = db.collection('category').find().sort({ priority: 1 }).toArray(function (err, doc) {
            if (err) {
                route && route([])
                db.close();
            } else if (!doc || doc.length == 0) {
...
var getAllCaseCategory = function (req, res, route) {
    mongoClient.connect(mogoUrl, function (err, db) {
        var cursor = db.collection('caseCategory').find().sort({ priority: 1 }).toArray(function (err, doc) {
            if (err) {
                route && route([])
                db.close();
            } else if (!doc || doc.length == 0) {
...
var getCategory = function (req, res, route) {
    mongoClient.connect(mogoUrl, function (err, db) {
        db.collection('category').find({ categoryId: req.query.categoryId.trim() }).sort({ priority: 1 }).toArray(function (err, doc) {
            if (err) {
                route && route({})
                db.close();
            } else if (!doc || doc.length == 0) {
...
var getIntegrator = function (req, res, route, query = null) {
    mongoClient.connect(mogoUrl, function (err, db) {
        db.collection('integrator').find(query ? query : { iid: req.query.iid.trim() }).sort({ priority: 1 }).toArray(function (err, doc) {
            if (err) {
                route && route([])
                db.close();
            } else if (!doc || doc.length == 0) {
...
var getCaseCategory = function (req, res, route) {
    mongoClient.connect(mogoUrl, function (err, db) {
        db.collection('caseCategory').find({ categoryId: req.query.categoryId.trim() }).sort({ priority: 1 }).toArray(function (err, doc) {
            if (err) {
                route && route({})
                db.close();
            } else if (!doc || doc.length == 0) {
...
var getAllProduct = function (req, res, route) {
    mongoClient.connect(mogoUrl, function (err, db) {
        var cursor = db.collection('product').find().sort({ priority: 1 }).toArray(function (err, doc) {
            if (err) {
                route && route([])
                db.close();
            } else if (!doc || doc.length == 0) {
...
var getCase = function (req, res, route, query = null) {
    mongoClient.connect(mogoUrl, function (err, db) {
        var cursor = db.collection('case').find(query ? query : {}).sort({ priority: 1 }).toArray(function (err, doc) {
            if (err) {
                route && route([])
                db.close();
            } else if (!doc || doc.length == 0) {
...
var getAllOrders = function (req, res, route) {
    mongoClient.connect(mogoUrl, function (err, db) {
        db.collection('order').find().sort({ inDate: -1 }).toArray(function (err, doc) {
            if (err) {
                route && route([]);
                db.close();
            }
...
var getOrder = function (req, res, route) {
    mongoClient.connect(mogoUrl, function (err, db) {
        db.collection('order').find({ oid: req.query.oid.trim() }).sort({ inDate: -1 }).toArray(function (err, doc) {
            if (err) {
                route && route({});
                db.close();
            }
...
var getBanner = function (req, res, route, query = null) {
    mongoClient.connect(mogoUrl, function (err, db) {
        db.collection('banner').find(query ? query : {}).toArray(function (err, doc) {
            if (err) {
                route && route([]);
                db.close();
            }
        ...
        db.collection('parts').find({ partsId: req.query.partsId }).toArray(function (err, doc) {
            if (err) {
                ...
                db.collection('parts').updateOne({ partsId: req.query.partsId }, {
                    ...
                    db.close();
                    // db.collection('product').find({parts: req.query.partsId }).toArray(function(err, doc){
                    //     if(err){
                    //         res.send({ status: 'success', msg: '修改成功' })
                    //         db.close();
                    //     }
        ...
        db.collection('training').find({ "ttid": req.query.ttid.trim() }).toArray(function (err, doc) {
            if (err) {
                ...
                db.collection('training').updateOne({ ttid: req.query.ttid.trim() }, {
                    $set: {
                        ...product,
                        "content": product.content != "<p>&nbsp;</p>" ? product.content : c.content,
                    },
                    $currentDate: { "lastModified": true }
                }, function () {
                    res.send({ status: 'success', msg: "修改成功" })
        ...
        db.collection('product').find({ "pid": product.pid.trim() }).toArray(function (err, doc) {
            if (err) {
                ...
                let a = {
                    ...
                    // "size": product.size,
                    "weight": product.weight,
                    "onSale": property.filter((x) => { return x.isHot == 1 }).length > 0 ? 1 : 0,
                    "store": {
                        "storeId": product.storeId ? parseInt(product.storeId) : -1,
                        "storeName": product.storeName.trim()
                    },
    ...
    mongoClient.connect(mogoUrl, function (err, db) {
        db.collection('order').find({ "oid": req.query.oid.trim() }).toArray(function (err, doc) {
            if (err) {
                ...
                let od = doc[0];
                // deliverStatusInfo: { delivers: [{ status: '提交订单', time: moment().format('YYYY-MM-DD HH:MM') }], company: {} },
                let deliverStatusInfo = Object.assign({}, od.deliverStatusInfo);
                let trackingInfo = Object.assign({}, od.trackingInfo);
                let needUpdateUser = false;
                if (trackingInfo.payInfo == undefined) {
                    trackingInfo.payInfo = {}
                }
                if (deliverStatusInfo.delivers && deliverStatusInfo.delivers.length > 0) {
                    let statusType = '';
                    ...
                    deliverStatusInfo.delivers.unshift({
                        status: statusType,
                        time: moment().format('YYYY-MM-DD HH:MM')
                    })
                }
                trackingInfo.payInfo.shippingChargeAmount = parseFloat(req.body.shippingChargeAmount) || 0;
                ...
                db.collection('order').updateOne({ "oid": req.query.oid.trim() }, {
                    ...
                    if (needUpdateUser) {
                        db.collection('user').find({ email: od.email.trim() }).toArray(function (err, doc) {
                            if (err) {
                                res.send({ status: 'success', msg: '修改成功' })
                                db.close();
                            }
                                ...
                                var totalConsume = doc[0].totalConsume + parseFloat(req.body.totalAmoumt);
                                db.collection('user').updateOne({ "email": od.email.trim() }, {
                                    $set: {
...
var updateBanner = function (req, res, common) {
    mongoClient.connect(mogoUrl, function (err, db) {
        db.collection('banner').find({ id: parseInt(common.id) }).toArray(function (err, doc) {
            if (err) {
                res.send({ status: 'failed', msg: '服务出错稍后再试' })
                db.close();
            } else if (!doc || doc.length == 0) {
                db.collection('banner').insertOne({
                    "id": parseInt(common.id),
                    "imgs": common.imgs,
                }, function () {
                    res.send({ status: 'success', msg: '修改成功' })
                    db.close();
                ...
                db.collection('banner').updateOne({ id: parseInt(common.id) }, {
                    $set: { "imgs": common.imgs },
                    $currentDate: { "lastModified": true }
                });
                res.send({ status: 'success', msg: '修改成功' })
                db.close();
...
var updatePriority = function (req, res, query, result, collection) {
    mongoClient.connect(mogoUrl, function (err, db) {
        db.collection(collection).updateOne(query, {
            $set: { ...result },
            $currentDate: { "lastModified": true }
