...
function buildMenuListView(data){
    let trs = '';
    data.forEach(function(child, index){
        trs+=`<li><a href="/productlist?storeId=${child.storeId}">${child.storeName}</a></li>`
    })
    
    let html = `<a href="/productlist">产品</a><ul class="nav-down">${trs}</ul>`
    return html;
}
...
module.exports.buildMenuListView = buildMenuListView;